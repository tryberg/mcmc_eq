...
paste t1 quakes.dat | awk '{print $3-$14}' | gmt pshistogram -JX -R -W0.1 -Z0 -G0 -K -O -V -F >> error.ps
...
paste t1 quakes.dat | awk '{print $4-$15}' | gmt pshistogram -JX -R -W0.1 -Z0 -G0 -K -O -V -F >> error.ps
...
paste t1 quakes.dat | awk '{print $5-$16}' | gmt pshistogram -JX -R -W0.1 -Z0 -G0 -K -O -V -F >> error.ps
...
paste t1 quakes.dat | awk '{print $9+$10-$17}' | gmt pshistogram -JX -R -W0.025 -Z0 -G0 -K -O -V -F >> error.ps
...
# true values
echo 0.05 | awk '{print $1, 0; print $1, 2000}' | gmt psxy -JX -R -W2,200/200/255 -O -K -N >> error.ps
...
echo 0.175 | awk '{print $1, 0; print $1, 2000}' | gmt psxy -JX -R -W2,255/200/200 -O -K -N >> error.ps
...
gmt psbasemap -JX2/2 -R-0.5/0.5/0/50 -B0.2f0.1g100:"Station correction P dt [s]":/0SWen -K -O -Y-3.0 -X-2.35 >> error.ps
...
echo -0.5 45  "m/s="$m"+-"$s" s" | gmt pstext -JX -R -K -O -N -F+jBL >> error.ps


# station correction S
gmt psbasemap -JX2/2 -R-0.5/0.5/0/50 -B0.2f0.1g100:"Station correction S dt [s]":/0SWen -K -O -X2.35 >> error.ps
...
# Vp
gmt psbasemap -JX1.2/-5 -R4/9/-5/80 -B1f0.5:"Vp [km/s]":/10f5g1000:"Depth [km]":Swen -K -O -X2.25 -Y0 >> error.ps
egrep EZ resmcnx.dat | awk '{print 4, $5}' | gmt psxy -JX -R -Sc0.05 -Ggreen -K -O >> error.ps

awk '{print $2, $1}' synth_model | gmt psxy -JX -R -W1,blue -K -O -N >> error.ps
awk '{if ($1=="STAN") print $7, $2}' resmcnx.dat | awk '{if (NR==1) {v0=$1;} print v0, $2; print $1, $2; v0=$1;}' | gmt psxy -JX -R -W1,red -O -K -N  >> error.ps
awk '{if ($1=="STAN") print $7-$8, $2}' resmcnx.dat | awk '{if (NR==1) {v0=$1;} print v0, $2; print $1, $2; v0=$1;}' | gmt psxy -JX -R -W,gray -O -K -N  >> error.ps
awk '{if ($1=="STAN") print $7+$8, $2}' resmcnx.dat | awk '{if (NR==1) {v0=$1;} print v0, $2; print $1, $2; v0=$1;}' | gmt psxy -JX -R -W,gray -O -K -N  >> error.ps

# Vp/Vs
gmt psbasemap -JX1.2/-5 -R1.501/2/-5/80 -B0.2f0.1:"Vp/Vs":/10f5g1000:"Depth [km]":SwEn -K -O -X1.35  >> error.ps
awk '{print $3, $1}' synth_model | gmt psxy -JX -R -W1,blue -K -O -N >> error.ps
awk '{if ($1=="STAN") print $9, $2}' resmcnx.dat | awk '{if (NR==1) {v0=$1;} print v0, $2; print $1, $2; v0=$1;}' | gmt psxy -JX -R -W1,red -O -K -N  >> error.ps
awk '{if ($1=="STAN") print $9-$10, $2}' resmcnx.dat | awk '{if (NR==1) {v0=$1;} print v0, $2; print $1, $2; v0=$1;}' | gmt psxy -JX -R -W,gray -O -K -N  >> error.ps
awk '{if ($1=="STAN") print $9+$10, $2}' resmcnx.dat | awk '{if (NR==1) {v0=$1;} print v0, $2; print $1, $2; v0=$1;}' | gmt psxy -JX -R -W,gray -O -K -N  >> error.ps
